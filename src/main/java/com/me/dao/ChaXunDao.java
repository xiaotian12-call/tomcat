...
public class ChaXunDao {
    ...
    public List<XvQiu> flcx(String xqzttj, String bytj, String bytjz, String sfmh, String qxsj, String jzsj) {
        ...
        if(xqzttj!=null&&!xqzttj.equals("99")){
            sql += " and sfsh = " +xqzttj;
        }
    ...
    public List<XvQiu> zhjs(String xqzttj,String sql, String qxsj, String jzsj) {
        QueryRunner qr = new QueryRunner(DBUtils.getDataSource());
        if(qxsj!=null&&!qxsj.equals("")){
            sql += " and date > '" +qxsj+"' and date < '"+jzsj+"'";
        }
...
//        System.out.println(jgsx.getJgsx());
    ...
    public List<Ktcs> ktcs(String tjzd, String n) {
        QueryRunner qr = new QueryRunner(DBUtils.getDataSource());
        String sql = "select distinct  * from ktcs order by ?  desc  limit "+n;
        //String sql = "select ? ?,count(*) num from t_dcwjxx where ? = ?";
        System.out.println(sql);
        List<Ktcs> query = null;
        try {
            query = qr.query(sql, new BeanListHandler<Ktcs>(Ktcs.class),tjzd);
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return query;
