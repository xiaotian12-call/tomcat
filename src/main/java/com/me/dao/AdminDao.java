...
public class AdminDao {
    ...
    public boolean changePower(String id, Power power) {
        QueryRunner qr =new QueryRunner(DBUtils.getDataSource());
        String sql="update power set power = ? , xqzj_qx = ? ,xqgl_qx=? ,yhxx_qx=? ,yhxg_qx=? ,xssh_qx=? ,mmcz_qx=? ,tjcx_qx=?,bmsh_qx=? where uid=? ";
        int n = 0;
        try {
            n = qr.update(sql,power.getPower(),power.getXqzj_qx(),power.getXqgl_qx(),power.getYhxx_qx(),power.getYhxg_qx(),power.getXssh_qx(),power.getMmcz_qx(),power.getTjcx_qx(),power.getBmsh_qx(),id);
        } catch (SQLException e) {
            e.printStackTrace();
        }
        if (n > 0) {
